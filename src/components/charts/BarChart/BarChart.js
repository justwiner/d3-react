...
class BarChart extends Component {
    ...
    addTitle(svg, barTitle, padding, contentWidth) {
        ...
        switch (barTitle.left) {
            case 'left' :
                title.attr('transform', `translate(${padding.left},${padding.top / 2})`);
                break
            case 'center':
                title.attr('transform', `translate(${contentWidth / 2 + padding.left},${padding.top / 2})`);
                break
            case 'right':
                title.attr('transform', `translate(${contentWidth + padding.left},${padding.top / 2})`);
                break
            default :
                title.attr('transform', `translate(${padding.left},${padding.top / 2})`);
                break
    ...
    addRect(svg, data, bar, xScale, yScale) {
        ...
        const rect = svg.selectAll('rect')
            ...
            .attr('x', (d, i) => {
                return xScale(axis.xAxisData[i]) + spacing
            })
            .attr('width', barWidth)
        if (animation) {
    ...
    // 添加纵向分割线
    addyLine(svg, xAxis, data, height) {
        // 定义轴网格线
    ...
    // 添加横向分割线
    addxLine(svg, yScale, data, contentWidth) {
    ...
    render() {
        return (
            <div id="barGraph" className="barGraph"></div>
        )
    }
}
